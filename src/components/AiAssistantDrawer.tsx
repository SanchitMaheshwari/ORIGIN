  ...
  return (
    <aside aria-label="FRA-MITRA AI Assistant" className="fixed bottom-4 right-4 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div
          id="ai-chat-window"
          className="mb-3 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-emerald-200 overflow-hidden flex flex-col transition-all duration-300 transform animate-in fade-in slide-in-from-bottom-3"
        >
          {/* Chat Header */}
          <div className="bg-gov-900 text-white p-3.5 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <div className="w-7 h-7 rounded-full bg-emerald-500 flex items-center justify-center text-white text-xs shadow">
                <Bot className="w-4 h-4" />
              </div>
              <div>
                <h4 className="text-xs font-bold leading-tight">Ask FRA-MITRA AI</h4>
                <span className="text-[10px] text-emerald-300">Ground-truth verified Decision Support</span>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="text-slate-300 hover:text-white text-sm p-1 rounded hover:bg-white/10 transition"
              title="Minimize chat"
          ...
          <div className="px-3 pt-2 pb-1 bg-slate-100/70 border-b border-slate-200 flex flex-wrap gap-1.5 overflow-x-auto text-[10px]">
            <button
              onClick={() => handleSend('Explain delay on Claim F-412')}
              className="px-2 py-0.5 bg-white rounded-full border border-slate-300 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-800 transition whitespace-nowrap cursor-pointer"
            >
              Delay reason F-412
            </button>
            <button
          ...
          {/* Messages Stream */}
          <div className="p-3 h-64 overflow-y-auto space-y-2.5 text-xs bg-slate-50 scrollbar-thin">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`text-xs ${
                  msg.sender === 'user'
                    ? 'bg-slate-200 text-slate-800 p-2.5 rounded-xl ml-auto max-w-[85%] text-right font-medium'
                    : msg.isAiInsight
                    ? 'bg-emerald-50 p-2.5 rounded-xl border border-emerald-200 text-slate-700 space-y-1 shadow-xs'
                    : 'bg-white p-2.5 rounded-xl border border-slate-200 shadow-xs text-slate-700 space-y-1'
                }`}
              >
                {msg.sender === 'bot' && (
                  <span className="text-[10px] text-gov-800 font-bold block flex items-center space-x-1">
                    {msg.isAiInsight ? (
                      <Sparkles className="w-3 h-3 text-emerald-600 inline mr-1" />
                    ) : (
                      <Info className="w-3 h-3 text-emerald-600 inline mr-1" />
                    )}
                    <span>{msg.isAiInsight ? 'Verified System Response:' : 'FRA-MITRA Knowledge Bot'}</span>
                  </span>
                )}
                <p className="leading-relaxed">{msg.text}</p>
                <span className="text-[9px] text-slate-400 block text-right pt-0.5">{msg.timestamp}</span>
              </div>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 text-xs border border-slate-300 rounded-lg py-1.5 px-3 focus:outline-none focus:ring-1 focus:ring-gov-800"
            />
            <button
              type="submit"
              className="bg-gov-800 hover:bg-gov-900 text-white px-3 py-1.5 rounded-lg text-xs font-semibold shadow flex items-center justify-center transition active:scale-95 cursor-pointer"
              title="Send message"
            >
          ...
          <div className="px-3 py-1 bg-slate-100 border-t border-slate-200 text-[9px] text-slate-400 text-center">
            Queries outside system database will return &quot;Insufficient Information&quot;.
          </div>
        </div>
      )}

      {/* Toggle Action Pill Button */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="bg-gov-900 hover:bg-gov-800 text-white px-4 py-2.5 rounded-full shadow-2xl flex items-center space-x-2.5 border-2 border-emerald-400 hover:scale-105 transition-all cursor-pointer"
      ...
      >
        ...
        <span className="text-xs font-bold tracking-wide">Ask FRA-MITRA</span>
